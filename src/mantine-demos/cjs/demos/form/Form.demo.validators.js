...
function Demo() {
  const form$1 = form.useForm({
    ...
    validate: {
      name: form.hasLength({ min: 2, max: 10 }, "Name must be 2-22 characters long"),
      job: form.isNotEmpty("Enter your current job"),
      email: form.isEmail("Invalid email"),
      favoriteColor: form.matches(/^#([0-9a-f]{3}){1,2}$/, "Enter a valid hex color"),
      age: form.isInRange({ min: 18, max: 99 }, "You must be 18-99 years old to register")
    }
  });
